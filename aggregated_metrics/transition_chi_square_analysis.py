...
def main() -> None:
	...
	print(f"Eligible participants (complete gaze in all conditions): {len(eligible_pids)}")
	...
	for _, trow in transitions.iterrows():
		...
		# Descriptive: pooled event counts across all participants (for reference only)
		pooled_patch = int(
			props_no_patch[
				(props_no_patch["from_aoi"] == from_aoi)
				& (props_no_patch["to_aoi"] == to_aoi)
				& (props_no_patch["condition"].isin(["correct", "overfitting"]))
				& (props_no_patch["PID"].isin(eligible_pids))
			]["proportion"].count()  # row count, not sum
		)
